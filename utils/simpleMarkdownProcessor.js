...
function translateMDtoHTMLDecoupled(md) {
    ...
    md = md.replace(/(^|\n)(`{3,}|~{3,})([^\n]*)\n([\s\S]*?)(?:\n\2(?=\n|$)|$)/g, (match, prefix, fence, info, code) => {
        ...
        const token = getMarkdownToken("CODEBLOCK", fencedCodeIndex);
        fencedCodeIndex += 1;
    ...
    const renderTable = (tableLines) => {
        if (tableLines.length < 2) return renderParagraph(tableLines);
        ...
        if (bodyRows.some((cells) => cells.length !== alignments.length)) return renderParagraph(tableLines);
        ...
        return `<table><thead><tr>${renderCells(headerCells, "th")}</tr></thead><tbody>${bodyRows.map((cells) => `<tr>${renderCells(cells, "td")}</tr>`).join("")}</tbody></table>`;
    };
    ...
    const renderBlocks = (blockLines) => {
        ...
        while (index < localLines.length) {
            ...
            if (heading) {
                html.push(`<h${heading[1].length}>${processInline(heading[2].trim())}</h${heading[1].length}>`);
                index += 1;
                continue;
            }
...
const copyButtonStates = new Map();
...
function scheduleCopyButtonUpdate() {
    ...
    requestAnimationFrame(() => {
        ...
        for (const [element, state] of copyButtonStates.entries()) {
...
function registerCopyButton(element) {
    if (!element || element.dataset.copyBtnRegistered === "1") return;
    element.dataset.copyBtnRegistered = "1";
...
async function highlightAllCodeBlocks() {
    ...
    // Prism's callback fires per highlighted element (and may not fire at all if there
    // are no matching code blocks). Ensure we always resolve.
    const selector =
    ...
    await new Promise((resolve) => {
        ...
        try {
            Prism.highlightAll(false, finish);
        } catch (e) {
            resolve();
        }

        // Safety net: never block callers (e.g., chat submission) indefinitely.
