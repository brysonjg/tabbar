...
const localDB = {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = async () => {
                ...
                try {
                    ...
                    console.warn("localDB migration failed, legacy ChatJson may remain:", err);
                }
                resolve(this._db);
            };
        });
    ...
    async migrateFromLocalStorageIfNeeded() {
        ...
        try {
            ...
            console.warn("ChatJson migration: parse failed, leaving localStorage intact.");
            return;
        }
        ...
        return new Promise((resolve, reject) => {
            const tx = openReadWriteTx(this._db, [
                STORE_SESSIONS,
                STORE_TITLES,
                STORE_META,
            ]);
            const sessions = tx.objectStore(STORE_SESSIONS);
            const titles = tx.objectStore(STORE_TITLES);
            const metaStore = tx.objectStore(STORE_META);

            const keys = Object.keys(parsed);
            for (let i = 0; i < keys.length; i++) {
                const key = keys[i];
                if (key === "activeTabID") continue;
                const sk = String(key);
                const val = parsed[key];
                sessions.put({ id: sk, payload: val });
                if (shouldSyncTitleRow(sk)) {
                    titles.put({ id: sk, title: titleFromSession(val) });
                }
            }

            if (parsed.activeTabID !== undefined && parsed.activeTabID !== null) {
                metaStore.put({
                    key: "activeTabID",
                    value: parsed.activeTabID,
                });
            }

            tx.oncomplete = () => {
                try {
                    localStorage.removeItem("ChatJson");
                } catch (e) {
                    console.warn("Could not remove ChatJson after migration:", e);
                }
                resolve();
            };
